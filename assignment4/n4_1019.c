...
int addNode( LIST *pList, tName *dataInPtr) {
    ...
    // 발견함 - 중복
    if (found) {
        int foundDataOut = searchList(pList, dataInPtr, dataOutPtr);
        fprintf(stderr, "%d %d\n", found, foundDataOut);
...
static int _insert( LIST *pList, NODE *pPre, tName *dataInPtr) {
    ...
    if (pPre == NULL) {
        ...
        //fprintf(stderr, "pPreNULL1\n");
        pNew->rlink = pList->head;
        
        //fprintf(stderr, "pPreNULL2\n");
		pList->head = pNew;
        
       // fprintf(stderr, "pPreNULL3\n");

        // 여기가 제일 이상함,,
        //pNew->llink->rlink = pNew;
        //fprintf(stderr, "pPreNULL4\n");
        
	}
	else {
        if (pPre->rlink = NULL) {
            pList->rear = pNew;
            //fprintf(stderr, "pPreNOTNULL1\n");
        }
		pNew->rlink = pPre->rlink;
        //fprintf(stderr, "pPreNOTNULL2\n");
		pPre->rlink = pNew;
        //fprintf(stderr, "pPreNOTNULL3\n");
        pNew->llink = pPre;
        //fprintf(stderr, "pPreNOTNULL4\n");
        // 여기도 이상함!!
        //pNew->llink->rlink = pNew;
        //fprintf(stderr, "pPreNOTNULL5\n");
	}
...
//			NULL if overflow
tName *createName( char *name, int freq) {
    tName *nName = (tName *) malloc (sizeof(nName));
    nName->name = (char *) malloc (sizeof(char) * 100);
    strcpy(nName->name, name);
    nName->freq = freq;
    return nName;
}
